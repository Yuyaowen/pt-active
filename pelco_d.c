...
/* Following is private command */
struct pelco_d *set_low_speed(unsigned char cam_addr)
{
    return set_preset(cam_addr, 15);
}

struct pelco_d *set_mid_speed(unsigned char cam_addr)
{
    return set_preset(cam_addr, 16);
}

struct pelco_d *set_high_speed(unsigned char cam_addr)
{
    return set_preset(cam_addr, 17);
}
